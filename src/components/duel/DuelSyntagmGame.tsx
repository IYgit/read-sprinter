...
interface MatchInfo {
  ...
  textTitle: string;
  textContent: string;
  questions: SyntagmQuestion[];
  totalCells: number; // = totalQuestions
}

interface DuelSyntagmGameProps {
  matchInfo: MatchInfo;
...
const DuelSyntagmGame = ({
  ...
  // ── Reading phase: auto-advance every displayTime ms ──────────────────────
  useEffect(() => {
  ...
  }, [phase, chunks.length, displayTime]);
  ...
  const handleAnswer = useCallback((questionId: number, optionIndex: number) => {
    ...
    setAnswers(prev => {
      ...
      if (answered === questions.length && !hasSaved.current) {
        hasSaved.current = true;
        const durationMs = Date.now() - startTimeRef.current;
        const score = calcSyntagmScore(correct, questions.length, displayTime, syntagmWidth);
  ...
  }, [answers, finished, questions, displayTime, syntagmWidth, onProgress, onFinish]);

  const readingProgress = chunks.length > 0
    ...
    return (
      <div className="space-y-4 animate-fade-in-up">
        ...
        <div className="glass-card p-6 lg:p-8">
          <p className="font-reading leading-[2.2] text-left text-lg">
            {words.map((word, index) => {
              const isHighlighted = currentChunk
                && index >= currentChunk.start
                && index < currentChunk.end;
...
export default DuelSyntagmGame;
